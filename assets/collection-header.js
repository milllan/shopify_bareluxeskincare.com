...
if (!customElements.get('m-collection-header')) {
  class MCollectionHeader extends HTMLElement {
    constructor() {
      super();
    }
    connectedCallback() {
      const parallaxEnabled = this.dataset.enableParallax === 'true';
      if (parallaxEnabled && window.innerWidth > 767) {
        const bg = this.querySelector('.collection-header__bg');
        loadAssets('parallax.js').then(() => {
          new MinimogLibs.simpleParallax(bg, {
            delay: 1.8,
  ...
  customElements.define('m-collection-header', MCollectionHeader);
}
}();
/******/ })()
;